...
namespace AppProjekt.ViewModels
{
    public class TemperatureViewModel : BaseViewModel
    {
        ...
        public ICommand UpdateChart => updateChart ??= new Command(
            execute: (args) =>
            {
                GenerateTempChart(Convert.ToInt32(args));
            });
        #endregion
        ...
        private void GenerateTempChart(int number = 0)
        {
            ...
            List<feeds> items = todaysfeeds.Where((x, i) => i % number == 0).ToList();     // Temperaturen opdateres per 10 sek. det er 360 gange i timen

            //List<feeds> items = feeds.Skip(Math.Max(0, feeds.Count() - 15)).ToList();

            foreach (var item in items)
            {
                entries.Add(new ChartEntry((float)Convert.ToDecimal(item.field1))
                {
                    Color = SkiaSharp.SKColor.Parse("#ff3c00"),
                    Label = item.created_at.ToString() /*.ToShortTimeString().Replace("AM","").Replace("PM","")*/,
                    ValueLabel = item.field1
                });
            }
